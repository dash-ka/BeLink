...
def filter_collections(args, collection):
        ...
        print("Loading and reformatting CTD-Disease...")
...
def main():
            ...
            passage.annotations = [ anno for anno in passage.annotations if \
                                   (anno.infons['MESH'] != '-1') and
                                     ('|' not in anno.infons['MESH']) and 
                                      ("CompositeRole" not in anno.infons) and (anno.infons['type'] == 'Disease') 
                                      ]
            for anno in passage.annotations:
    ...
    print("Saving documents BC5CDR Disease ...")
    #save_bioc_docs(train_docs_bc5cdr, output_dir / "train.bioc.xml.gz" )
    #save_bioc_docs(val_docs_bc5cdr, output_dir / "val.bioc.xml.gz")
