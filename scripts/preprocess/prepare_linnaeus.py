import argparse, os, json, re, bioc
...
def filter_collection(args, collection):
    ...
    cui_set = load_cui_set(os.path.join(args.dictionary_dir, "train_dictionary.txt"))
    print("CUI set size: ", len(cui_set))

    with open(os.path.join(args.dictionary_dir, "alt_ids2cui.json")) as f:
...
def main():
    parser = argparse.ArgumentParser(description='Convert Linnaeus to BioCXML')
    parser.add_argument('--dictionary_dir',required=True,type=str,help='Path to disambiguated terminology.')
    ...
    for idx, item in tqdm(enumerate(corpus["train"]), total=len(corpus["train"])):
        ...
        bioc_doc.id = item["document_id"]
        bioc_passage = bioc.BioCPassage()

        bioc_passage.text = sanitize_xml_text("".join(p["text"][0] for p in item["passages"]))
        bioc_passage.offset = 0
        bioc_doc.add_passage(bioc_passage)

        for a in item["entities"]:
            bioc_anno = bioc.BioCAnnotation()
            bioc_anno.infons['concept_id'] = a["normalized"][0]["db_id"]
            bioc_anno.text = a["text"][0]
            start, end = a["offsets"][0]
    ...
    print("Saving Linnaeus documents...")
